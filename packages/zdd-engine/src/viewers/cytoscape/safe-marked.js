...
const safeMarked = (() => {
  ...
  // A protocol-relative target (`//host/p`, and the `\\host`, `/\host`,
  // `\/host` forms browsers read the same way) has no scheme of its own yet
  // leaves the site: it is an off-site link in disguise, not a relative one
  // (CR-106) — plain text like any other unknown scheme.
  const safeHref = (href) => {
