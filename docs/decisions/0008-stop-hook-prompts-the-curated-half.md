...
Both hosts fire a `Stop` hook when the agent ends a turn, and both honour a `decision: "block"` JSON reply on exit 0 (the same shape lesson as decision 0007: Codex ignores exit 2). Claude Code sends `stop_hook_active: true` when it is already continuing because a Stop hook blocked; both hosts send a `session_id`.
